...
def concat_houses(data, houses_list=None, window_selection=None, include_fake_breakpoint=False):
    ...
    for i in range(0, len(houses_list)):
        ...
        if include_fake_breakpoint:
            # 50 50 distribution
            fake_breakpoints = np.zeros(y1_part)
...
if TRAIN:
    ...
    if IMPROVED:
        dump(best_bi_model, '../models/' + TEST_DATA + '/improved/breakpointidentifier' + str(best_accuracy_bi) + ".ml")
        dump(best_sl_model, '../models/' + TEST_DATA + '/improved/segmentlabeler' + str(best_accuracy_sl) + ".ml")
    else:
        dump(best_bi_model, '../models/' + TEST_DATA + '/original/breakpointidentifier' + str(best_accuracy_bi) + ".ml")
        dump(best_sl_model, '../models/' + TEST_DATA + '/original/segmentlabeler' + str(best_accuracy_sl) + ".ml")
